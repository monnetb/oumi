...
#set -eux 
export REQUESTS_CA_BUNDLE=$HOME/certif.crt # Zscaler_Root_CA.pem
...
echo "Using ${YAML_FILE} as config file"
...
echo "Going to use ${OUMI_ROOT}/${TGT} as Python environement"
...
srun --ntasks=${SLURM_NNODES} --ntasks-per-node=1 --nodes=${SLURM_NNODES} oumi distributed torchrun --log-level DEBUG -m oumi train  -c $PWD/${YAML_FILE}
